	...
	public static String toUpper(String s) {
		...
		for (int i = 0; i < sb.length(); i++) { 
			...
			if (sb.charAt(i) <= 122 && sb.charAt(i) >= 97) { // compreso >0 && <=
				...
				char d = (char) c;
				String x=Character.toString(d); // meglio cambiare nome della stringa. COSI' ho cambiato un carattere in string
...
/*
  public static String toUpper2(string s) {
  	Stringbuilder sb= new StringBuilde(s) // allora ho fatto una stringa modificabile = ad s. è come il mio scrivere bob
 for( int=0;i<s.lenght();i++) {
  char cur = s.charAt(i);
  if ( cur>= 'a' && cur <='z' ){
...
/*
 * public static void main(String[] args) { // passo da B a intero String s =
 * "bHeLLo";
 * 
 ...
 * prima char d = (char) c; System.out.print(d); } else {
